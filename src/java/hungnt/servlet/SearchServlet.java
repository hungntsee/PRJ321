    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (searchValue.trim().length() > 0) {
                ...
                List<userDTO> dto = dao.getList();
                request.setAttribute("LIST_DTO", dto);
            }

        } catch (ClassNotFoundException ex) {
            log("SearchServlet-ClassNotFound error: " + ex.getMessage());
            //ex.printStackTrace();
        } catch (SQLException ex) {
            log("SearchServlet-SQL error: " + ex.getMessage());
            //ex.printStackTrace();
        } catch (NamingException ex) {
            log("SearchServlet-Naming error: " + ex.getMessage());
            //ex.printStackTrace();
        } finally {
